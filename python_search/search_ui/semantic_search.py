...
class SemanticSearch:
    ...
    def __init__(self, number_entries_to_return=None):
        self.setup()
        self.number_entries_to_return = number_entries_to_return if number_entries_to_return else 15
    ...
    def setup_entries(self):
        ...
        for entry in tqdm.tqdm(entries, total=loader.entries_total()):
            collection.upsert(
                documents=[entry.key + ' ' + entry.get_content_str() + entry.get_type_str()],
                ids=[entry.key],
            )
    ...
    def search(self, query: str):
        results = self.collection.query(query_texts=[query], n_results=self.number_entries_to_return)["ids"][0]
        return results
