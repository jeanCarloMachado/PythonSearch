...
class TermUI:
        ...
        while True:
            if self.query:
                ...
                self.matches = self.get_bm25().get_top_n(
                    tokenized_query, entries, n=NUMBER_ENTTRIES_RENDER
                )
            else:
    ...
    def setup_documents(self):
        tokenized_corpus = [
            tokenize((key + str(value))) for key, value in self.commands.items()
        ]
...
def print_normal_row(key, entry, tokenized_query):
    ...
    body_part = cf.entrycontentunselected(
        control_size(
            entry.get_content_str(strip_new_lines=True).strip(), MAX_CONTENT_SIZE
        )
    )
