...
class Pipeline:
    ...
    def run(self, embeddings_cache=None, cache=False, disable_arize=False):
        ...
        if not disable_arize:
            ...
            for key, entry in enumerate(x_val):
                value, label = get_value_and_label(model.predict(np.asarray([entry]))[0])
                result = self._arize_client.log(
                    ...
                    embedding_features={'content': Embedding(vector=entry, data=input_list[key])},
    ...
    def create_dataset(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame(dataset, columns=['input', 'has_pipe', 'has_double_minus', 'label'])
        return df
