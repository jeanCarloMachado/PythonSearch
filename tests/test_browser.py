...
class TestBrowser(unittest.TestCase):
    ...
    def test_open_chrome_on_mac(self):
        ...
        def mock_is_linux():
            return False
        b = Browser(system_func=mock_system, is_mac_func=mock_is_mac, is_linux_func=mock_is_linux)
    ...
    def test_fail_safe_no_supported_browser(self):
        ...
        def mock_is_linux():
            return False
        b = Browser(system_func=None, is_mac_func=mock_is_mac, is_linux_func=mock_is_linux)
        with self.assertRaises(Exception) as context:
            b.open(url="http://example.com")
        self.assertTrue("No supported browser found. Please install chrome/firefox or customize your browser in python_search/apps/browser.py" in str(context.exception))
