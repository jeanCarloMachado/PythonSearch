...
class SearchAndRunCli:
    ...
    def edit_key(self, key):
        ...
        """
        if not key:
            self._edit_config(self.configuration.get_source_file())
            return
    ...
    def _edit_config(self, file_name: str, line: Optional[int] = 30):
        ...
        s.run(
            f"MY_TITLE='GrimorieSearchRun' runFunction terminal_run 'cd {self.configuration.get_project_root()} ; vim {file_name} +{line}' ",
        )
